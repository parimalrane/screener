...
from asta_conditions import (
    is_macd_pco, is_macd_nco, is_macd_rising,
    is_rsi_above, is_price_above_sma, is_stochastic_buy,
    is_bkp, is_bbuc, is_solid_candle, is_ema_pco, is_volume_above_average
)
...
def check(df_daily: pd.DataFrame, df_weekly: pd.DataFrame, df_monthly: pd.DataFrame):
    try:
        ...
        # -----------------------------------------------------------------
        tags = ""
        if is_bkp(df_daily) or is_bbuc(df_daily): tags += "*"
